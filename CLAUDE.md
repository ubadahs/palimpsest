...
npm run dev -- runs:gc [--run-id <uuid>] [--dry-run]  # delete superseded run artifacts + unreferenced provenance
...
- **Boundary validation**: All external data (API responses, env vars, LLM outputs, XML) is Zod-validated before entering the domain layer. Types are inferred from Zod schemas (`z.infer<typeof schema>`). Each stage builds its boundary parser from `createBoundaryParser` in `src/shared/boundary.ts`; deterministic ordering and deduplication come from `src/shared/order.ts`.
- **Artifact integrity, once**: `src/contract/lean-artifacts.ts` is a barrel over `src/contract/artifacts/{envelope,discover,scope,prepare,evidence,checks,seed-section-role}.ts`. Two checks apply to every artifact: its identity hash over payload + decisions, and that it names the exact inputs it was built from in canonical order (`validateStageInputLineage`). Per-stage lineage validators that re-derived what a builder had just written are gone; each stage keeps only its payload's internal-consistency ledger.
...
- **Canonical Report**: Report tamper-verifies the full preceding chain and deterministically writes authoritative JSON funnel/rate/trace accounting plus Markdown rendered only from that JSON. Funnel accounting includes unique citation-group coverage, the provider-reported neighborhood total and its coverage (so the sampling denominator is stated, and rates over it are unweighted), full-text access channel, materialization and harvest loss reasons, bibliography match method, adaptive portfolio deferrals, and manual-review queue splits. `selectionAudit` carries each candidate's portfolio component scores and each record trace carries its citation-role signals. F/D/E/U rates use the adjudicated-record denominator; operational non-verdicts are excluded.
- **Clean break for old runs**: Migration `0011_purge_pre_canonical_runs.sql` plus startup config validation delete unsupported analysis-run registry rows. Migration `0012_drop_orphan_papers_citations.sql` drops unused `papers`/`citations` tables, and `0013_drop_write_only_columns.sql` drops `derived_artifacts` plus every column that was written and never read back (`analysis_runs.tracked_claim`, `analysis_run_stages.input_artifact_path`/`exit_code`, the `paper_cache` metadata columns, `paper_parsed.sections_json`, and `llm_result_cache.purpose`/`model`/`key_version`). `analysis_run_stages.family_index` stays because it is part of an immutable primary key. Paper/LLM caches and on-disk `data/runs/` files are preserved; old runs are not resumed or bridged. `db:gc` deletes aged analysis-run and LLM-cache SQLite rows only (not artifact directories); `runs:gc` deletes superseded stage attempts a `--rerun-from` left on disk plus the provenance blobs only those attempts referenced.
