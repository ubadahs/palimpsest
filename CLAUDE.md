...
- **Vector-first adjudicator** (opt-in): `adjudicationMode: "vector_first"` runs `fidelityVectorTrace` before categorical adjudication, adaptively adds simple borderline samples, accepts clear `axisDerivedVerdict` outputs as final axis-derived verdicts, and escalates risky records to the existing categorical adjudicator using original unmodified audit records. The default remains `adjudicationMode: "categorical"`; in vector-first mode do not run the post-hoc diagnostic trace path after vector-first tracing.
