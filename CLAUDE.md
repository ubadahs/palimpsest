...
- **Canonical Report (isolated, not executor-wired, deterministic)**: `runCanonicalReport` (`src/pipeline/canonical-report.ts`) is a pure transformation of the complete current canonical chain (Discover → Scope → Prepare → Evidence → Adjudicate). It tamper-verifies one run ID and exact artifact ID/content-hash links across all five ancestors, binds all five as direct report inputs in that fixed order, and emits authoritative JSON funnel counts, explicit rate objects with recomputed numerator/denominator/value (null when denominator is zero), one per-record trace, and decision/exclusion summaries. The contract enforces funnel partitions, unique ordered status summaries, required metric numerator/denominator semantics, and numerator≤denominator. Discover citing-paper counts are seed-specific observations, not globally unique papers. Evidence reports BM25/reranked final selections both as unique selection objects and as per-record uses; these are distinct populations. Report has exactly two deterministic five-input-bound decisions (interpretation and publication status) and no Report-stage exclusions. Evidence trace retrieval/rerank statuses reuse the cycle-free exact schemas in `canonical-evidence-statuses.ts`; per-record adjudication failure codes are nonfatal-only. F/D/E/U rates use the adjudicated-record denominator only; adjudication coverage uses all Prepare/Evidence records; operational non-verdicts never enter verdict counts; U stays separate from gates/failures. No adapters, LLM calls, PDF/HTML, headline quality score, or evaluation/benchmark/agreement statistics. Contracts live in `src/contract/canonical-report.ts` (re-exported from lean-artifacts). `renderCanonicalReportMarkdown` formats validated JSON only and must not derive independent counts/rates. `writeCanonicalReportArtifacts` validates/writes JSON first, then renders Markdown from that exact parsed object; `loadCanonicalReportArtifact` is current-JSON-only. Prepare low-information/manual-review populations can overlap classification statuses and are not a partition. Blinded calibration and separate evaluation reports remain outstanding; CLI/executor wiring remains deferred.
