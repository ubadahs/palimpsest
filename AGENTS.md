...
- **Adjudication advisor mode**: set `adjudicateAdvisor: true` (or `--advisor` on CLI) for a two-pass strategy — cheap Sonnet first pass on all records, Opus+thinking only on `judgeConfidence === "low"` or `cannot_determine` escalations. Saves ~50-70% of adjudication cost on well-grounded families. Disabled by default.
