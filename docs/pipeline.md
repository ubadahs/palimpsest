...
Every canonical model call uses provider-enforced structured output (`generateObject`) against the adapter's own response schema, so a malformed reply is no longer a failure class; what can still fail is a candidate number that names no chunk, which stays a typed `invalid_output`. Each request body stores its prompt text once, so any call can be reproduced from the artifact alone.
