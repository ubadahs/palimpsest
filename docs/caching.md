...
When `attribution_first` is the discovery strategy, `runDiscoveryStage` produces a `DiscoveryHandoffMap` for downstream stages. Fresh execution serializes and reconstructs it through the same current schema persisted under `inputs/discovery-handoffs.json`; resume requires that validated sidecar. This eliminates the most expensive redundant work:
...
Missing, unreadable, invalid, tampered, or shortlist-incomplete handoff bundles fail attribution-first resume. The bundle is reusable provenance for `screen` and `extract`, not a replacement for stage primary JSON artifacts.
