...
- **Side-by-side model calls.** Extraction, grounding, reranking, and adjudication run `modelConcurrency` requests at once (default 6, `--model-concurrency`). The third 2026-09-02 replay ran sequentially and spent 47 of its 67 minutes in Adjudicate; the same run should take about a quarter of that. Artifacts are identical at any concurrency; only wall time changes.
